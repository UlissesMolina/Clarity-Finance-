...
function downloadPDF(rows: TransactionDetail[]) {
  const rowsHtml = rows
    .map(
      (t) =>
        `<tr><td>${t.date}</td><td>${(t.description || '').replace(/</g, '&lt;')}</td><td>${t.category}</td><td>${t.type}</td><td>${formatCurrency(t.amount, { sign: true })}</td><td>${(t.notes || '').replace(/</g, '&lt;')}</td></tr>`
    )
    .join('');
...
export function Transactions({ className }: TransactionsProps) {
  ...
  const handleExportPDF = () => downloadPDF(selectedIds.size > 0 ? filteredList.filter((t) => selectedIds.has(t.id)) : filteredList);
